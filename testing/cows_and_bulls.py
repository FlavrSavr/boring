...
def cows_and_bulls():
    ...
    user_input = list(input("I'm thinking of a four digit number. Type your guess and then hit 'enter': "))
        ...
        try:
            ...
            for position,element in enumerate(user_input):
                if element in remaining_numbers:
                    ...
                    user_input[position] = "!"
        print("*"*80)
        user_input = list(input("Not quite, that's "+str(bull_count)+" bull(s) and "+str(cow_count)+" cow(s). Try again with a new guess. Your last guess was "+("".join(preserved_user_input))+". "))
        guess_count += 1
        continue
    if user_input == my_number and guess_count == 1:
        ...
        print("*"*80)
        print("Good job, the number was "+("".join(my_number))+"! You got it in "+str(guess_count)+" guesses!")
